...
function menuToggle() {
  if (curQuiz === 'none' && Object.keys(quiz).includes(answer.value))
    startQuiz(answer.value);
  else {
...
function create() {
  ...
  limbo.insertAdjacentHTML(
    'beforeend',
    `<p class='limbo-name'>NAME : ${n} <br><br></p>`
  );
  quiz[n] = [];
}

function add() {
  let question = quizQuestion.value;
  let answer = quizAnswer.value;
  inProcess = quiz[n];
  let html = `<p class='limbo-question'>QUESTION: ${
    inProcess.length + 1
  } <br> </p><p class='limbo-data'>${question}</p><p class='limbo-data'> ANSWER: ${answer}<br><br></p><div class=line></div>`;
  if (question === '' || question === 'number') return;
  inProcess.push([question, answer]);
